...
const QueryAnalytics: React.FC<QueryAnalyticsProps> = ({
  ...
  const truncateQuery = (query: string, maxLength?: number) => {
    const defaultLength = isMobile ? 30 : 50;
      ...
      <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 1, sm: 2 }, flexWrap: 'wrap' }}>
            <Search color="primary" sx={{ fontSize: { xs: '1.25rem', md: '1.5rem' } }} />
            <Typography 
            ...
            >
              {isMobile ? "Query Analytics" : "Query Analytics & Search Patterns"}
            </Typography>
            {analytics && (
              <Chip
                label={isMobile ? `${analytics.top_queries.length} Queries` : `${analytics.top_queries.length} Top Queries`}
                color="primary"
                size="small"
              ...
              <Grid container spacing={{ xs: 1.5, sm: 2, md: 3 }} sx={{ mb: { xs: 3, md: 4 } }}>
                <Grid item xs={6} sm={6} md={3}>
                  <Paper sx={{ 
                    ...
                    >
                      {isMobile ? "Popular" : "Popular Queries"}
                    </Typography>
                  </Paper>
                </Grid>
                <Grid item xs={6} sm={6} md={3}>
                ...
                <Grid item xs={6} sm={6} md={3}>
                  <Paper sx={{ 
                    ...
                    <Typography 
                      ...
                      sx={{ fontSize: { xs: '0.75rem', sm: '0.875rem' } }}
                    >
                      {isMobile ? "Collections" : "Active Collections"}
                    </Typography>
                  </Paper>
                </Grid>
                <Grid item xs={6} sm={6} md={3}>
                  <Paper sx={{ 
                    ...
                    <Typography 
                      ...
                      sx={{ fontSize: { xs: '0.75rem', sm: '0.875rem' } }}
                    >
                      {isMobile ? "Avg Terms" : "Avg Terms/Query"}
                    </Typography>
                  </Paper>
                </Grid>
              </Grid>
              ...
              <Accordion defaultExpanded={!isMobile}>
                <AccordionSummary expandIcon={<ExpandMore />}>
                  ...
                  >
                    ...
                    {isMobile ? "Top Queries" : "Top Queries & Performance"}
                  </Typography>
                </AccordionSummary>
                <AccordionDetails sx={{ p: { xs: 1, sm: 2 } }}>
                  {isMobile || viewMode === 'cards' ? (
                    <Stack spacing={2}>
                      {analytics.top_queries.slice(0, isMobile ? 5 : 10).map((query, index) => (
                            ...
                            <Box sx={{ mb: 2 }}>
                              ...
                              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                                {query.collections && query.collections.length > 0 ? (
                                  query.collections.slice(0, isMobile ? 1 : 2).map((collection, idx) => (
                                    <Chip 
                                      key={idx} 
                                      label={collection} 
                                      size="small" 
                                ...
                                {query.collections && query.collections.length > (isMobile ? 1 : 2) && (
                                  <Chip 
                                    label={`+${query.collections.length - (isMobile ? 1 : 2)}`} 
                                    size="small" 
                                    variant="outlined" 
                                    color="default"
                                    sx={{ fontSize: '0.65rem' }}
                            ...
                            <Grid container spacing={1}>
                              ...
                              <Grid item xs={6}>
                                ...
                                <Typography 
                                  variant="body2" 
                                  sx={{ fontWeight: 600 }}
                                  color={getPerformanceColor(query.avg_response_time_ms)}
                                >
                              ...
                              <Grid item xs={6}>
                                ...
                                <Typography 
                                  variant="body2" 
                                  sx={{ fontWeight: 600 }}
                                  color={query.success_rate_percent > 95 ? 'success.main' : 'warning.main'}
                                >
                              ...
                              {!isMobile && (
                                <Grid item xs={12}>
                                  <Typography variant="caption" color="text.secondary">
                                    Peak Hour: {query.peak_usage_hour}:00
                                  </Typography>
                                </Grid>
                              )}
                            </Grid>
