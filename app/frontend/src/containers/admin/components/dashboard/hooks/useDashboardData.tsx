...
interface UseDashboardDataReturn {
  ...
  isDataFresh: boolean;
  fetchComprehensiveStats: (options?: {
    useCache?: boolean;
    force?: boolean;
  }) => Promise<void>;
  fetchEmbeddingDetails: (options?: {
    useCache?: boolean;
    force?: boolean;
  }) => Promise<void>;
  fetchAdvancedStats: (options?: {
    useCache?: boolean;
    force?: boolean;
  }) => Promise<void>;
...
export const useDashboardData = (
  ...
  const [embeddingDetails, setEmbeddingDetails] =
    useState<DetailedEmbeddingStats | null>(null);
  const [advancedStats, setAdvancedStats] = useState<AdvancedCacheStats | null>(
    null,
  );
   ...
   */
  const fetchComprehensiveStats = useCallback(
    async (
   ...
   */
  const fetchEmbeddingDetails = useCallback(
    async (
      options: { useCache?: boolean; force?: boolean } = {},
    ): Promise<void> => {
      const { useCache = true, force = false } = options;
   ...
   */
  const fetchAdvancedStats = useCallback(
    async (
      options: { useCache?: boolean; force?: boolean } = {},
    ): Promise<void> => {
      const { useCache = true, force = false } = options;
  ...
  useEffect(() => {
    if (accessToken && isMountedRef.current) {
      ...
      if (
        !isDataFresh(CACHE_KEYS.COMPREHENSIVE) &&
        !isDataFresh(CACHE_KEYS.LEGACY)
      ) {
  ...
  const currentDataFresh =
    isDataFresh(CACHE_KEYS.COMPREHENSIVE) || isDataFresh(CACHE_KEYS.LEGACY);
