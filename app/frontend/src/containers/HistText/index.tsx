...
const HistText: React.FC = React.memo(() => {
        ...
        const promises = data.solrDatabases.map(async (database) => {
          try {
            ...
            if (Array.isArray(response.data)) {
              ...
              console.warn(
                `Invalid response for database ${database.id}:`,
                response.data,
              );
  ...
  const handleSolrDatabaseChangeWithHistory = useCallback(
    async (database: any) => {
      if (!database) {
      ...
      if (data.authAxios && databaseToSet.id) {
        try {
          ...
          if (Array.isArray(response.data)) {
            ...
            console.log(
              `Loaded ${response.data.length} aliases for database ${databaseToSet.id}`,
            );
          } else {
          ...
          console.error(
            "Error fetching aliases for database:",
            databaseToSet.id,
            error,
          );
          data.setAliases([]);
          showNotification(
            "Failed to fetch collections for selected database",
            "error",
          );
        }
      }
  ...
  const handleAliasChangeWithHistory = useCallback(
    async (alias: string) => {
      console.log(
        "Changing alias to:",
        alias,
  ...
  const handleSwitchAndApplySearch = useCallback(
    async (search: any) => {
      try {
        ...
        const targetDatabase = data.solrDatabases.find(
          (db) => db.id === search.selectedSolrDatabase.id,
        );
        if (!targetDatabase) {
          throw new Error(
            `Database with ID ${search.selectedSolrDatabase.id} not found`,
          );
        }

        // Validate that the target collection exists for that database
        const collectionsForDb =
          allCollections[search.selectedSolrDatabase.id] || [];
        if (!collectionsForDb.includes(search.selectedAlias)) {
          throw new Error(
            `Collection "${search.selectedAlias}" not found in database "${targetDatabase.name}"`,
          );
        }
        ...
        if (search.selectedSolrDatabase.id !== data.selectedSolrDatabase?.id) {
          console.log(
            "Switching database from",
            data.selectedSolrDatabase?.id,
            "to",
            search.selectedSolrDatabase.id,
          );
        ...
        if (search.selectedAlias !== data.selectedAlias) {
          console.log(
            "Switching alias from",
            data.selectedAlias,
            "to",
            search.selectedAlias,
          );
        ...
        showNotification(
          `Applied search "${search.name}" successfully`,
          "success",
        );
      } catch (error) {
