...
const EntityTimeline: React.FC<EntityTimelineProps> = ({ stats, entities }) => {
  ...
  const timelineData = useMemo((): TimelineDataPoint[] => {
    ...
    return stats.documentStats.slice(0, 30).map((doc: any, index: number) => {
      const estimatedDocLength = doc.entityCount * 50;
      const entityDensity =
        (doc.entityCount / Math.max(estimatedDocLength, 1000)) * 1000;

      const entityTypes = doc.entityTypes || {};
      const dominantType = Object.entries(entityTypes).reduce(
        (max, [type, count]) =>
  ...
  const entityEvolutionData = useMemo(() => {
    ...
    entities
      .filter(
        (e) =>
          e.text.toLowerCase().includes(selectedEntity.toLowerCase()) ||
          e.normalizedText
            ?.toLowerCase()
    ...
    return timelineData
      .map((point) => ({
        ...
        relativeFrequency: entityOccurrences.get(point.documentId)
          ? (entityOccurrences.get(point.documentId)! / point.entityCount) * 100
          : 0,
      }))
  ...
  const timelineTrends = useMemo(() => {
    ...
    const avgFirst = {
      entityCount:
        firstHalf.reduce((sum, d) => sum + d.entityCount, 0) / firstHalf.length,
      uniqueEntities:
        firstHalf.reduce((sum, d) => sum + d.uniqueEntities, 0) /
        firstHalf.length,
      diversityIndex:
        firstHalf.reduce((sum, d) => sum + d.diversityIndex, 0) /
        firstHalf.length,
    };

    const avgSecond = {
      entityCount:
        secondHalf.reduce((sum, d) => sum + d.entityCount, 0) /
        secondHalf.length,
      uniqueEntities:
        secondHalf.reduce((sum, d) => sum + d.uniqueEntities, 0) /
        secondHalf.length,
      diversityIndex:
        secondHalf.reduce((sum, d) => sum + d.diversityIndex, 0) /
        secondHalf.length,
    };

    return {
      entityCountTrend:
        avgSecond.entityCount > avgFirst.entityCount
          ? "increasing"
          : "decreasing",
      diversityTrend:
        avgSecond.diversityIndex > avgFirst.diversityIndex
          ? "increasing"
          : "decreasing",
        ...
        <Collapse in={showExplanation}>
          <Alert severity="info" sx={{ mb: 2 }}>
            <Typography variant="body2">
              ...
              <br />
              1. <strong>Document Sequence:</strong> Documents ordered by
              processing sequence (may correlate with temporal patterns)
              <br />
              2. <strong>Entity Density:</strong> Entities per estimated
              document length (normalized)
              <br />
              3. <strong>Diversity Index:</strong> Shannon entropy of entity
              types, normalized (0-1, higher = more diverse)
              <br />
              4. <strong>Dominant Type:</strong> Most frequent entity type in
              each document
              <br />
              5. <strong>Entity Evolution:</strong> How specific entities
              appear/disappear over sequence
              <br />
              <br />
              <strong>Trends:</strong> Computed by comparing first and second
              half of document sequence
            </Typography>
          </Alert>
        </Collapse>
