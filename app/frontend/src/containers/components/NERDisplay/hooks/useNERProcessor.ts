...
export const useNERProcessor = () => {
  ...
  const processAdvancedStats = useCallback(async (
    ...
    try {
      ...
      const shouldDoFullAnalysis = entities.length <= processingConfig.MAX_ENTITIES_FOR_FULL_ANALYSIS;
      ...
      let currentStats: NERAdvancedStats = {
        ...
        averageEntitiesPerDocument: entities.length / basicStats.totalDocuments,
      ...
      if (currentStats.documentStats.length > 0) {
        ...
        currentStats.documentsWithHighestDiversity = currentStats.documentStats
          .slice()
          .sort((a, b) => {
            const diversityA = a.uniqueEntityCount / Math.max(a.entityCount, 1);
            const diversityB = b.uniqueEntityCount / Math.max(b.entityCount, 1);
            return diversityB - diversityA;
          })
          .slice(0, 10);

        currentStats.anomalyScores = StatisticsComputer.computeAnomalyScores(currentStats.documentStats);
      }
      ...
      if (shouldDoRelationships && shouldDoFullAnalysis) {
        ...
        try {
          ...
          console.log(`Relationship analysis completed: ${cooccurrences.length} relationships found`);

          currentStats.entityCooccurrences = cooccurrences.slice(0, 100);
          currentStats.strongestPairs = cooccurrences.slice(0, 20);
          currentStats.centralityScores = StatisticsComputer.computeCentralityScores(
            topEntities, 
            cooccurrences
          );
          ...
          currentStats.centralityScores = StatisticsComputer.computeCentralityScores(topEntities, []);
          
          updateProcessingState({
            phase: 'relationships',
            progress: 65,
            currentTask: 'Relationship analysis skipped (dataset too complex)'
          });
        }
      } else {
        console.log('Skipping relationship analysis - dataset too large or full analysis disabled');
        updateProcessingState({
          phase: 'relationships',
          progress: 65,
      ...
      if (shouldDoPatterns && shouldDoFullAnalysis) {
        ...
        console.log('Skipping pattern analysis - dataset too large or full analysis disabled');
        updateProcessingState({
          phase: 'patterns',
          progress: 90,
      ...
      console.log('Advanced processing completed successfully:', {
        ...
        patterns: currentStats.bigramPatterns.length + currentStats.trigramPatterns.length + currentStats.quadrigramPatterns.length,
