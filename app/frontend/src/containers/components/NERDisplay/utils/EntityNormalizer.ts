...
export class EntityNormalizer {
  ...
  private static readonly PREFIX_REGEX =
    /^(mr\.|mrs\.|ms\.|dr\.|prof\.|president|minister|chairman|director|chief|head|deputy|vice|the|a|an)\s+/i;
  private static readonly SUFFIX_REGEX =
    /\s+(inc\.?|corp\.?|ltd\.?|llc|co\.?|company|corporation|limited|group|international|intl)$/i;
  ...
  static shouldFilter(
    ...
    if (normalizedText.length <= 1 || normalizedText.length > 100) {
      ...
      !["DATE", "TIME", "MONEY"].includes(label)
    ) {
      shouldFilter = true;
      ...
      ["http", "https", "www", "html", "pdf"].some((artifact) =>
        normalizedText.includes(artifact),
      )
    ) {
      shouldFilter = true;
    }
