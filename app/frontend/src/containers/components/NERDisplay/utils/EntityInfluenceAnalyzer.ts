...
export class EntityInfluenceAnalyzer {
  ...
  static computeEntityInfluence(
    ...
    entityGroups.forEach((group) => {
      ...
      const entityConnections = cooccurrences.filter(
        (cooc) => cooc.entity1 === entityText || cooc.entity2 === entityText,
      );
      const centralityScore =
        entityConnections.length / Math.max(entityGroups.size - 1, 1);
      ...
      documentsByEntity.forEach((doc1) => {
        documentsByEntity.forEach((doc2) => {
          if (doc1 !== doc2) {
            const doc1Entities = entities
              .filter((e) => e.documentId === doc1)
              .map((e) => e.normalizedText);
            const doc2Entities = entities
              .filter((e) => e.documentId === doc2)
              .map((e) => e.normalizedText);
            const sharedEntities = doc1Entities.filter((e) =>
              doc2Entities.includes(e),
            ).length;
      ...
      const bridgeScore =
        bridgeConnections / Math.max(documentReach * (documentReach - 1), 1);

      const cooccurrenceStrength =
        entityConnections.length > 0
          ? entityConnections.reduce(
              (sum, cooc) => sum + (cooc.strength || 0),
              0,
