...
const NERDisplay: React.FC<NERDisplayProps> = ({
  ...
  const processedData = useMemo(() => {
   ...
   return allEntities
     .filter(entity => {
       ...
       const matchesLabel = selectedLabel === 'all' || entity.label === selectedLabel;
 ...
 }, [nerData, searchTerm, selectedLabel, minConfidence, sortBy, sortOrder]);
 ...
 const entityStats = useMemo(() => {
   ...
   processedData.forEach(entity => {
     stats.byLabel[entity.labelFull] = (stats.byLabel[entity.labelFull] || 0) + 1;
   });
           ...
           >
             ...
             {uniqueLabels.map(label => (
               <MenuItem key={label} value={label}>
                 {config.NERLABELS2FULL[label] || label}
               </MenuItem>
             ))}
           </Select>
         </FormControl>
   ...
   <Paper sx={{ p: 3, mb: 3, borderRadius: 2 }}>
     ...
     <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
       {Object.entries(entityStats.byLabel)
         .sort(([,a], [,b]) => b - a)
         .map(([label, count]) => {
           const color = config.NER_LABELS_COLORS[Object.keys(config.NERLABELS2FULL).find(key => config.NERLABELS2FULL[key] === label)] || '#grey';
           return (
             <Chip
               key={label}
               label={`${label} (${count})`}
               ...
               onClick={() => setSelectedLabel(Object.keys(config.NERLABELS2FULL).find(key => config.NERLABELS2FULL[key] === label) || 'all')}
             />
           );
         })}
     </Box>
