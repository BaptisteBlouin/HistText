...
export const useSearchHistory = (options: UseSearchHistoryOptions = {}) => {
  ...
  const saveAsBookmark = useCallback(
    (
      search: SavedSearch | Omit<SavedSearch, "id" | "createdAt" | "lastUsed">,
    ) => {
      const bookmark: SavedSearch = {
        ...
        id:
          "id" in search
            ? search.id
            : `bookmark_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        createdAt:
          "createdAt" in search ? search.createdAt : new Date().toISOString(),
        lastUsed: new Date().toISOString(),
        isBookmarked: true,
      };
  ...
  const searchInSaved = useCallback(
    (query: string) => {
  ...
  const importSearches = useCallback(
    (file: File) => {
        ...
        reader.onload = (event) => {
          try {
            ...
            if (data.history && Array.isArray(data.history)) {
              setSearchHistory((prev) =>
                [...data.history, ...prev].slice(0, maxHistorySize),
              );
            }

            if (data.bookmarks && Array.isArray(data.bookmarks)) {
              setBookmarks((prev) =>
                [...data.bookmarks, ...prev].slice(0, maxBookmarksSize),
              );
            }
