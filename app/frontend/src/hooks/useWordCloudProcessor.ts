...
const isMeaningfulWord = (word: string, isChinese: boolean): boolean => {
  ...
  if (isChinese) {
    ...
    return /[\u4e00-\u9fff]/.test(trimmed) && trimmed.length >= 1 && trimmed.length <= 4;
  } else {
    // For non-Chinese: require at least 2 characters
    return trimmed.length >= 2 && trimmed.length <= 25 && /[a-zA-Z]/.test(trimmed);
  }
};
...
const fallbackTokenize = (text: string): string[] => {
  ...
  if (hasChinese) {
    ...
    console.log('Fallback extracted Chinese words (first 10):', words.slice(0, 10));
  ...
  const computeCloudOptimized = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < texts.length; i += batchSize) {
        ...
        console.log(`Attempting API call for batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(texts.length / batchSize)}`);
        ...
        try {
          ...
          if (data.results && Array.isArray(data.results)) {
            ...
            data.results.forEach((result: any, idx: number) => {
              if (result.words && Array.isArray(result.words)) {
                console.log(`Result ${idx} has ${result.words.length} words:`, result.words.slice(0, 10));
                ...
                result.words.forEach((word: string) => {
                  if (word && typeof word === 'string') {
                    ...
                    if (hasChineseContent) {
                      ...
                      if (/[\u4e00-\u9fff]/.test(normalizedWord) && normalizedWord.length >= 1 && normalizedWord.length <= 4) {
                        wordMap.set(normalizedWord, (wordMap.get(normalizedWord) || 0) + 1);
                      }
                      // Also accept English words in mixed content
                      else if (/[a-zA-Z]/.test(normalizedWord) && normalizedWord.length >= 2 && normalizedWord.length <= 25) {
                        wordMap.set(normalizedWord.toLowerCase(), (wordMap.get(normalizedWord.toLowerCase()) || 0) + 1);
                      }
                    } else {
          ...
          apiFailureCount++;
          console.error(`API call failed for batch ${Math.floor(i / batchSize) + 1}:`, err);
      ...
      console.log(`API processing complete. Successes: ${apiSuccessCount}, Failures: ${apiFailureCount}`);
      ...
      console.log('Final word frequency data (first 20):', wordFrequencyData.slice(0, 20));

      setWordFrequency(wordFrequencyData);
      setCloudProgress(100);

      const apiStatus = apiSuccessCount > 0 ? 'with API processing' : 'with fallback processing only';
