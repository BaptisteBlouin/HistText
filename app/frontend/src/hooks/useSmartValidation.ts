    ...
    default: [
      {
        ...
        message:
          "Highlighting will not work for search terms shorter than 2 characters.",
        severity: "warning",
      },
      {
    ...
    text_field: [
      {
        test: (value) => !value.includes('"'),
        message:
          "Quotes are not needed - exact matching is handled automatically",
        severity: "error",
      },
      {
  ...
  const validateField = useMemo(() => {
    return (fieldName: string, entries: any[]): FieldValidation => {
      ...
      const hasMultipleEntries =
        entries.filter((e) => e.value?.trim()).length > 1;
      if (hasMultipleEntries) {
        const operators = entries.map((e) => e.operator).filter(Boolean);
        if (operators.length === 0) {
          suggestions.push(
            "Consider using AND/OR operators for multiple terms",
          );
        }
      }
