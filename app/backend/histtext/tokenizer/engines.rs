...
use super::utils::{
    is_likely_latin_safe, safe_take_chars, safe_truncate_string, CHINESE_STOPWORDS,
    ENGLISH_STOPWORDS, JIEBA,
};
...
pub fn tokenize_latin_ultra_fast(text: &str, cloud: bool, max_tokens: usize) -> Vec<String> {
    ...
    if cloud {
        for word in text.split_whitespace() {
            ...
            if !cleaned.is_empty() && cleaned.len() < 20 && !ENGLISH_STOPWORDS.contains(&cleaned) {
                tokens.push(cleaned);
                count += 1;
            }
        }
