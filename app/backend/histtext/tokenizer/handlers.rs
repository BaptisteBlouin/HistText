...
use super::types::{
    BatchTokenizeRequest, BatchTokenizeResponse, TokenizeRequest, TokenizeResponse,
};
...
pub async fn tokenize(params: web::Json<TokenizeRequest>) -> Result<HttpResponse, Error> {
    ...
    let words =
        std::panic::catch_unwind(|| tokenize_text_ultra_fast(&text, cloud)).unwrap_or_else(|_| {
...
pub async fn batch_tokenize(
    params: web::Json<BatchTokenizeRequest>,
) -> Result<HttpResponse, Error> {
    let BatchTokenizeRequest {
        texts,
        cloud,
        max_tokens_per_text,
    } = params.into_inner();
    ...
    let results = std::panic::catch_unwind(|| batch_tokenize_parallel(&texts, cloud, max_tokens))
        .unwrap_or_else(|_| {
