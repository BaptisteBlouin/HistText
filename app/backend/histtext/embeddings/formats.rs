...
async fn load_with_finalfusion_binary(
    ...
    match embeddings_result {
        ...
        Ok(Err(e)) => Err(EmbeddingError::Format(format!(
            "FinalFusion binary error: {}",
            e
        ))),
...
async fn convert_finalfusion_embeddings(
    embeddings: finalfusion::prelude::Embeddings<
        finalfusion::vocab::SimpleVocab,
    ...
    info!(
        "Converting finalfusion embeddings: {} words, {} dimensions",
        vocab_len, dimension
    );
...
async fn convert_finalfusion_fasttext_embeddings(
    embeddings: finalfusion::prelude::Embeddings<
        finalfusion::vocab::SubwordVocab<finalfusion::compat::fasttext::FastTextIndexer>,
        finalfusion::storage::NdArray,
    >,
    config: &EmbeddingConfig,
) -> EmbeddingResult<(EmbeddingMap, usize)> {
    ...
    info!(
        "Converting FastText embeddings: {} words, {} dimensions",
        vocab_len, dimension
    );
    ...
    info!(
        "Successfully converted {} FastText embeddings",
        our_embeddings.len()
    );
    Ok((our_embeddings, dimension))
}
...
async fn convert_finalfusion_embeddings_wrapped(
    embeddings: finalfusion::prelude::Embeddings<
        finalfusion::vocab::VocabWrap,
        finalfusion::storage::StorageWrap,
    >,
    config: &EmbeddingConfig,
) -> EmbeddingResult<(EmbeddingMap, usize)> {
    use finalfusion::prelude::*;
    ...
    let words = match vocab {
        ...
        _ => {
            return Err(EmbeddingError::UnsupportedFormat(
                "Unsupported vocab type".to_string(),
            ))
        }
    };
    ...
    info!(
        "Converting wrapped embeddings: {} words, {} dimensions",
        vocab_len, dimension
    );
    ...
    info!(
        "Successfully converted {} wrapped embeddings",
        our_embeddings.len()
    );
    Ok((our_embeddings, dimension))
}
