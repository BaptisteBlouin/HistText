    ...
    T: Send + 'static,
{
    ...
    if let Some(ref name) = tx_name {
        if duration > Duration::from_millis(200) {
            ...
            debug!("Database transaction '{}' completed in {:?}", name, duration);
        }
...
/// # Returns
/// Result containing the operation result or an AppError
pub async fn execute_db_transaction<T, F>(
    db: web::Data<Database>,
    operations: F,
) -> AppResult<T>
where
...
fn handle_diesel_error(error: DieselError, context: &str) -> AppError {
    match error {
        ...
        DieselError::DatabaseError(kind, info) => {
            ...
            match kind {
                diesel::result::DatabaseErrorKind::UniqueViolation => {
                    ...
                    warn!("Unique constraint violation on field '{}' in operation: {}", field, context);
                    AppError::validation(
                ...
                diesel::result::DatabaseErrorKind::ForeignKeyViolation => {
                    ...
                    warn!("Foreign key violation on relation '{}' in operation: {}", relation, context);
                    AppError::validation(
                ...
                diesel::result::DatabaseErrorKind::CheckViolation => {
                    ...
                    warn!("Check constraint violation '{}' in operation: {}", check_name, context);
                    AppError::validation(
                ...
                diesel::result::DatabaseErrorKind::NotNullViolation => {
                    ...
                    warn!("Not null violation on column '{}' in operation: {}", column, context);
                    AppError::validation(
                ...
                _ => {
                    error!("Database error ({:?}) in operation {}: {}", kind, context, info.message());
        ...
        DieselError::RollbackTransaction => {
            error!("Transaction explicitly rolled back in operation: {}", context);
            AppError::database(format!("Transaction rolled back in {}", context))
        },
        
        DieselError::AlreadyInTransaction => {
            warn!("Attempted to start a transaction while already in one: {}", context);
            AppError::database("Nested transactions are not supported".to_string())
        },
        
        // Handle other Diesel errors
        _ => {
            error!("Unhandled database error in operation {}: {:?}", context, error);
...
where
    F: FnOnce(&mut Connection) -> Result<Box<dyn std::any::Any + Send>, DieselError> + Send + 'static,
{
    ...
    for (i, operation) in operations.into_iter().enumerate() {
        ...
        let future = async move {
            execute_db_query_named(
                db_clone,
                move |conn| operation(conn),
                Some(&op_name)
            ).await
        };
    ...
    for (i, result) in results.iter().enumerate() {
        if let Err(e) = result {
            ...
            return Err(AppError::database(
                format!("Parallel query {} failed: {}", i, e)
            ));
        }
    }
    
    // Unwrap all successful results
    let unwrapped: Vec<_> = results.into_iter()
        .map(|r| r.unwrap())
        .collect();
    
    debug!("Successfully completed {} parallel database operations", op_count);
    Ok(unwrapped)
}
...
mod tests {
    ...
    async fn test_execute_db_query() {
        ...
        let result = execute_db_query(db.clone(), |conn| {
            ...
            diesel::sql_query("SELECT 1 as id")
                .load::<TestRow>(conn)
        }).await;
    ...
    async fn test_transaction_rollback() {
        ...
        let result = execute_db_transaction(db.clone(), |conn| {
            ...
            diesel::sql_query("SELECT invalid_column")
                .load::<TestRow>(conn)
        }).await;
