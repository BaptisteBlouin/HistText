...
impl RoleAssignmentService {
    ...
    pub async fn ensure_default_role_exists(
        &self,
            ...
            move |conn| {
                ...
                if existing_permissions == 0 {
                    ...
                    let default_permissions =
                        vec![crate::services::role_permissions::NewRolePermission {
                    ...
                    info!(
                        "Created default role '{}' with basic permissions",
                        DEFAULT_USER_ROLE
                    );
                } else {
                    debug!(
                        "Default role '{}' already exists with {} permissions",
    ...
    pub async fn assign_default_role_to_user(
            ...
            move |conn| {
                ...
                if existing_role.is_some() {
                    debug!(
                        ...
                        target_user_id, DEFAULT_USER_ROLE
                    );
                    return Ok(());
                }
                ...
                info!(
                    "Successfully assigned role '{}' to user {} due to {}",
        ...
        if let Err(e) = SecurityEventLogger::log_event(
            ...
            &format!(
                "Automatically assigned role '{}' due to {}",
                DEFAULT_USER_ROLE, trigger_reason
            ),
            "medium",
            req,
        )
    ...
    pub async fn remove_default_role_from_user(
        ...
        debug!(
            "Removing default role '{}' from user {}",
            DEFAULT_USER_ROLE, target_user_id
        );
        ...
        if deleted_count > 0 {
            ...
            if let Err(e) = SecurityEventLogger::log_event(
                ...
                Some(user_email_owned),
                &format!(
                    "Automatically removed role '{}' due to account deactivation",
                    DEFAULT_USER_ROLE
                ),
                "medium",
                req,
            ...
            debug!(
                "User {} did not have role '{}' to remove",
                target_user_id, DEFAULT_USER_ROLE
            );
        }
    ...
    pub async fn get_users_missing_default_role(
        ...
        info!(
            "Found {} users missing default role '{}'",
            missing_users.len(),
            DEFAULT_USER_ROLE
        );
        Ok(missing_users)
    ...
    pub async fn batch_assign_default_role(
        ...
        info!(
            "Batch assigning default role '{}' to {} users",
            DEFAULT_USER_ROLE,
            user_ids.len()
        );
        ...
        if let Err(e) = SecurityEventLogger::log_event(
            ...
            None,
            &format!(
                "Batch assigned role '{}' to {} users",
                DEFAULT_USER_ROLE, user_count
            ),
            "medium",
            req,
        )
