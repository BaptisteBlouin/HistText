...
impl Auth {
    ...
    pub fn has_permission(&self, permission: String) -> bool {
        self.permissions.iter().any(|p| p.permission == permission)
    }
...
impl FromRequest for Auth {
    ...
    fn from_request(req: &HttpRequest, _: &mut Payload) -> Self::Future {
        let config = match req.app_data::<web::Data<Arc<Config>>>() {
            Some(config) => config,
            None => {
                warn!("Server configuration not found in request");
            ...
            if let Ok(auth_str) = auth_header.to_str() {
                if let Some(token) = auth_str.strip_prefix("Bearer ") {
                    ...
                    if let Ok(token_data) = token_result {
                        ...
                        debug!("Successfully authenticated user {}", auth.user_id);
                        return ready(Ok(auth));
                    }

                    debug!("Invalid JWT token provided");
...
impl AuthHandler {
    ...
    pub async fn create_tokens(
        ...
        let (permissions, roles) = execute_db_query(db, move |conn| {
            ...
            debug!("Found {} roles for user {}", user_roles.len(), user_id);
    ...
    pub async fn login(
        ...
        debug!("Attempting login for user: {}", login_data.email);

        let email_for_query = login_data.email.clone();
    ...
    pub async fn change_password(
        ...
        debug!("Password change request for user: {}", auth.user_id);
    ...
    pub async fn reset_password(
        ...
        .map_err(|e| {
            ...
            AppError::auth(AuthErrorReason::InvalidToken, "Invalid reset token")
        })?;
    ...
    pub async fn activate(
        ...
        let token_data = decode::<RefreshTokenClaims>(
            &activation_data.activation_token,
            &DecodingKey::from_secret(self.config.secret_key.as_ref()),
            &Validation::default(),
        )
        .map_err(|e| {
            warn!("Invalid activation token: {}", e);
            AppError::auth(AuthErrorReason::InvalidToken, "Invalid activation token")
        })?;
    ...
    pub async fn refresh(
        ...
        .map_err(|e| {
            ...
            AppError::auth(AuthErrorReason::InvalidToken, "Invalid token")
        })?;
