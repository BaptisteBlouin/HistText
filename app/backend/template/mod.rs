...
pub mod spa;
mod vite_bundle;
pub use spa::render_single_page_application;
...
pub fn to_template_name(request_path: &str) -> &'static str {
    ...
    if request_path.is_empty() {
        ...
        println!("Non-empty path - using default template: {}", DEFAULT_TEMPLATE);
...
pub async fn render_views(req: HttpRequest) -> HttpResponse {
    ...
    if path.contains("@vite") || path.contains("@react-refresh") || 
       path.contains("node_modules/.vite") || path.contains("/__vite_ping") ||
       path.starts_with("/bundles/") || path.starts_with("/src/") {
        #[cfg(debug_assertions)]
        {
            println!("Detected Vite/HMR request. Proxying to Vite dev server: {}", path);
    ...
    match TEMPLATES.render(template_path, &Context::new()) {
        Ok(content) => {
            ...
            #[cfg(debug_assertions)]
            {
                ...
                let enhanced_content = if content.contains("<head>") {
                    ...
                    content.replace("<html>", &format!("<html><head>{}</head>", dev_client_injection))
                } else {
                    println!("Wrapping with full HTML structure");
                    format!("<html><head>{}</head>{}</html>", dev_client_injection, content)
                };
        ...
        Err(e) => {
            ...
            return HttpResponse::NotFound().body(format!("Template error: {}", e));
        }
    }
}
...
async fn proxy_to_vite_server(path: &str) -> Result<HttpResponse, Box<dyn std::error::Error>> {
    ...
    let headers = response.headers()
        ...
        .filter_map(|(key, value)| {
            value.to_str().ok().map(|v| (key.as_str().to_owned(), v.to_owned()))
        })
