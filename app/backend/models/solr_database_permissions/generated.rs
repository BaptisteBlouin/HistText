...
impl SolrDatabasePermission {
    pub fn create(db: &mut Connection, item: &CreateSolrDatabasePermission) -> QueryResult<Self> {
        ...
        insert_into(solr_database_permissions)
            .values(item)
            .get_result::<Self>(db)
    }
    ...
    pub fn read(
        db: &mut Connection,
        param_solr_database_id: i32,
        param_collection_name: String,
        param_permission: String,
    ) -> QueryResult<Self> {
        use crate::schema::solr_database_permissions::dsl::*;

        solr_database_permissions
            .filter(solr_database_id.eq(param_solr_database_id))
            .filter(collection_name.eq(param_collection_name))
            .filter(permission.eq(param_permission))
            .first::<Self>(db)
    }
    ...
    pub fn paginate(
        ...
        let items = solr_database_permissions
            .limit(page_size)
